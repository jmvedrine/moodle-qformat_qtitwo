...
<assessmentItem xmlns="http://www.imsglobal.org/xsd/imsqti_item_v2p1" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xsi:schemaLocation="http://www.imsglobal.org/xsd/imsqti_item_v2p1 ./imsqti_item_v2p1.xsd" identifier="{$assessmentitemidentifier}" title="{$assessmentitemtitle}" adaptive="false" timeDependent="false">
	<responseDeclaration identifier="{$questionid}" cardinality="single" baseType="identifier">
		<correctResponse>
			<value>i{$correctresponse.id}</value>
		</correctResponse>
		<mapping defaultValue="0">
			<mapEntry mapKey="i{$correctresponse.id}" mappedValue="{$correctresponse.fraction}"/>
		</mapping>
	...
	<itemBody>
        ...
        {section name=answer loop=$answers}
            ...
            {if $answers[answer].feedback != ''}
                ...
                <feedbackInline identifier="i{$answers[answer].id}" outcomeIdentifier="FEEDBACK" showHide="hide">{$answers[answer].feedback}</feedbackInline>
                {/if}
            {/if}
            </simpleChoice>
        {/section}
	...
	{section name=answer loop=$answers}
        {if $answers[answer].feedback != ''}
	<modalFeedback outcomeIdentifier="FEEDBACK" identifier="i{$answers[answer].id}" showHide="hide">{$answers[answer].feedback}</modalFeedback>
    	{/if}
	{/section}
</assessmentItem>
